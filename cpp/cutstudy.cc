...
void opt1d_bins_fixEff(float effTarget, TString expr, bool highIsGood) {
    ...
    TH2 *hS = (TH2*) gROOT->FindObject("htemp")->Clone("hS");
    if (expr.Contains(">>htemp")) btree->Draw(expr, background, "", nMax);
    else btree->Draw(expr+TString::Format(">>htemp(%d,%g,%g,%d,%g,%g)", hS->GetNbinsX(), hS->GetXaxis()->GetXmin(), hS->GetXaxis()->GetXmax(),hS->GetNbinsY(), hS->GetYaxis()->GetXmin(), hS->GetYaxis()->GetXmax()), background, "", nMax);
    TH2 *hB = (TH2*) gROOT->FindObject("htemp")->Clone("hB");
